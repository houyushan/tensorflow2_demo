...
if __name__ == '__main__':
    ...
    def train_step(images, labels):
        with tf.GradientTape() as tape:
            ...
            loss_aux = loss_object(
                y_true=labels, y_pred=predictions.aux_logits)
            loss = 0.5 * loss_aux + 0.5 * \
                loss_object(y_true=labels, y_pred=predictions.logits)
    ...
    for epoch in range(config.EPOCHS):
        ...
        # valid_accuracy.update_state(train_accuracy)
        step = 0
